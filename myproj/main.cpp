...
void menu(int item)
{
	switch(item)
	{
		...
		case MENU_SPLITEDGE:
		{
			map<myHalfedge *, myVertex *> center_map;
			map<pair<myVertex*, myVertex *>, myHalfedge *> twin_map;

			for(auto &e:m->halfedges)
			{
				if(center_map.find(e) == center_map.end())
				{
					myVertex *v = new myVertex();
					*(v->point) = m->edge_point(e);
					center_map.insert(make_pair(e, v));
					center_map.insert(make_pair(e->twin, v));
					twin_map.insert(make_pair(make_pair(e->source, e->twin->source), e));
				}
			}
			for(auto &t:twin_map)
			{
				m->splitEdge(t.second, center_map.find(t.second)->second);
			}
			m->computeNormals();
			break;	
		}
		case MENU_SPLITFACE:
		{
			vector<myFace *> copy_faces = m->faces;
			for(auto &f:copy_faces)
			{
				myVertex *v = new myVertex;
				*v->point =  m->face_point(f);
				m->splitFace(f, v);
			}
			m->computeNormals();
			m->checkMesh();
